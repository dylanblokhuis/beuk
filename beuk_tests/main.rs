...
fn test_capacity_with_vulkan_buffers(
...
fn test_capacity(ctx: Arc<RenderContext>) {
    let resource_manager = Arc::new(ResourceManager::<Test>::new(Arc::new(ctx.device.clone()), ctx.allocator.clone(), 100));

    let mut handles = vec![];
    for i in 0..20000 {
      let id = resource_manager.create(Test { yo: i, _pad: [0; 12]});
      handles.push(
        ResourceHandle::new(
          id,
